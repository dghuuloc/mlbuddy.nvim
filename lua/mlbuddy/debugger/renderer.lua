...
local function section_gradients(data, lines, hls, W)
  ...
  push(lines, hls,
    string.format("  %-32s  %10s  %10s  %8s  %s",
      "Parameter", "Grad Norm", "Grad Max", "Bar", "Status"),
    { { c0=0, c1=-1, hl="MlbuddyDim" } })
  push_rule(lines, hls, W, "╌")

  for _, g in ipairs(gf) do
    local gn    = type(g.grad_norm) == "number" and g.grad_norm or 0
    local bar_v = max_norm > 0 and (gn / max_norm) or 0
    local bar   = mini_bar(bar_v, 10)
    ...
    local line = string.format("  %-32s  %10s  %10s  %s  %s",
      g.name:sub(1,32),
      fmt(g.grad_norm), fmt(g.grad_max),
      bar, status)
    push(lines, hls, line, {
...
function M.build_lines(data, view, cfg)
  ...
  push(lines, hls,
    string.format("  󰃤  Model Debugger  ─  %s  (total: %s  trainable: %s)",
      model_info, total_p, train_p),
  ...
  if data.error then
    push(lines, hls, "  Error: " .. data.error,
      { { c0=0, c1=-1, hl="MlbuddyError" } })
    goto footer
  end
  if data.forward_error then
    push(lines, hls, "  Forward pass failed: " .. data.forward_error,
      { { c0=0, c1=-1, hl="MlbuddyWarn" } })
  end
